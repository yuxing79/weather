MODULE module_cumulus_python_data
...
CONTAINS
   ...
   USE module_cumulus_python_data
   ...
   cps_select: SELECT CASE(cu_physics)
     ...
     CASE (MSKFSCHEME)
          ...
          CALL MSKF_CPS(                                        &
               ...
               ,UER_KF=uer_kf,DER_KF=der_kf                     & 
               ,TIMEC_KF=timec_kf,KF_EDRATES=kf_edrates         & 
               ,ZOL=zol,HFX=hfx,UST=ust,PBLH=pblh               & 
               ,AEROCU=aerocu,NO_SRC_TYPES_CU=no_src_types_cu   &
               ,AERCU_FCT=aercu_fct,AERCU_OPT=aercu_opt        &
               ,EFCS=EFCS,EFIS=EFIS,EFSS=EFSS                   &
               ,RUCUTEN=RUCUTEN,RVCUTEN=RVCUTEN,XLAND=XLAND)       
!      call wrf_debug(100,'in python')
       if(mod(int(itimestep*dt),1800).eq.0)then
       RAINCV(2,:)=13.5
